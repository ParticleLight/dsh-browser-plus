        ...
        switch (op) {
            ...
            case 'showView': {
                ...
                for (const v of views.values()) {
                    if (v === entry || v.window !== entry.window)
                        continue;
                    try {
            ...
            case 'label': {
                ...
                try {
                    entry.window.setTitle(label === '' ? 'dsh-browser-plus' : `dsh-browser-plus — ${label}`);
                }
                catch { /* closing */ }
            ...
            case 'listWindows': {
                const windows = [];
                if (defaultWindow !== undefined && !defaultWindow.isDestroyed())
                    windows.push({ key: 'default', label: windowLabels.get('default') ?? '' });
                for (const [key, win] of windowsByKey) {
                    if (!win.isDestroyed())
                        windows.push({ key, label: windowLabels.get(key) ?? '' });
                }
            ...
            case 'capture': {
                ...
                if (base64 === '') {
                    ...
                    for (const v of siblings) {
                        try {
                            entry.window.contentView.removeChildView(v.webContentsView);
                        }
                        catch { /* already gone */ }
                    }
                    try {
                        ...
                        for (const v of siblings) {
                            try {
                                entry.window.contentView.addChildView(v.webContentsView);
                            }
                            catch { /* destroyed */ }
                        }
                        try {
