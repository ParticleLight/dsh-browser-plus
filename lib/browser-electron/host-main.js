...
function chromeStateScript(selectedTaskKey = visibleTaskKey) {
...
function pushChromeState(target, selectedTaskKey = visibleTaskKey) {
    try {
        void target.webContents.executeJavaScript(chromeStateScript(selectedTaskKey)).catch(() => undefined);
    }
    catch { /* closing */ }
}
function pushTaskState(target, selectedTaskKey) {
...
function pushVisibleChromeState() {
    ...
    const target = viewId === undefined ? undefined : views.get(viewId);
    if (target !== undefined)
        pushTaskState(target.webContentsView, visibleTaskKey);
}
...
async function refreshVisibleTaskThumbnail(taskKey) {
    ...
    try {
        ...
        if (taskKey !== visibleTaskKey || activeViewByTask.get(taskKey) !== viewId)
            return;
