...
import { createBootstrap, createPatch } from './chrome-state.js';
...
const taskViewIds = new Map();
const taskThumbnails = new Map();
const taskThumbnailVersions = new Map();
const taskStates = new Map();
const thumbnailTimers = new Map();
const thumbnailDirty = new Set();
let thumbnailCaptureInFlight = false;
const thumbnailLastCapturedAt = new Map();
...
let workspacePanels = { tasks: false, trail: false };
let window;
...
function ensureTaskState(key) {
    const existing = taskStates.get(key);
    if (existing !== undefined)
        return existing;
    const state = { status: 'idle', control: 'agent', updatedAt: Date.now() };
    taskStates.set(key, state);
    return state;
}
function updateTaskState(key, update) {
    const state = ensureTaskState(key);
    if (update.status === 'idle' || update.status === 'running' || update.status === 'waiting-user' || update.status === 'failed')
        state.status = update.status;
    if (update.control === 'agent' || update.control === 'human')
        state.control = update.control;
    if (typeof update.latestAction === 'string')
        state.latestAction = update.latestAction.slice(0, 120);
    if (typeof update.error === 'string')
        state.error = update.error.slice(0, 180);
...
let pendingChromeOperations = [];
let chromePatchTimer;
function activeTraceForTask(taskKey) {
    ...
    return entries.flatMap(entry => {
        ...
        return [{
                ...
                ...typeof record.params === 'object' && record.params !== null && !Array.isArray(record.params) ? { params: record.params } : {},
                ...typeof record.ok === 'boolean' ? { ok: record.ok } : {},
                at: record.at,
            }];
    });
}
function chromeWorkspaceState(selectedTaskKey = visibleTaskKey) {
    return {
        epoch: chromeEpoch,
...
function chromeBootstrapScript(selectedTaskKey = visibleTaskKey) {
...
function chromePatchScript(operations) {
...
function pushVisibleChromeState() {
    resetChromeDelivery();
    const viewId = visibleTaskKey === undefined ? undefined : activeViewByTask.get(visibleTaskKey);
    const target = viewId === undefined ? undefined : views.get(viewId);
    if (target !== undefined)
        pushChromeBootstrap(target.webContentsView, visibleTaskKey);
}
function flushChromePatches() {
    chromePatchTimer = undefined;
...
function queueChromePatch(...operations) {
    pendingChromeOperations.push(...operations);
    if (chromePatchTimer !== undefined)
        return;
    chromePatchTimer = setTimeout(flushChromePatches, 24);
}
function scheduleVisibleTaskThumbnail(taskKey, delayMs = 360) {
    if (taskKey !== visibleTaskKey)
        return;
...
async function refreshVisibleTaskThumbnail(taskKey) {
    if (taskKey !== visibleTaskKey || !workspacePanels.tasks || thumbnailCaptureInFlight)
        return;
    ...
    try {
        ...
        if (taskKey !== visibleTaskKey || activeViewByTask.get(taskKey) !== viewId || !workspacePanels.tasks)
            return;
        ...
        if (taskThumbnails.get(taskKey) === thumbnail)
            return;
        ...
        const operations = [{ op: 'task.thumbnail', key: taskKey, version, dataUrl: thumbnail }];
        if (task !== undefined)
        ...
        switch (op) {
            ...
            case 'trace': {
                ...
                if (entryView !== undefined) {
                    ...
                    if (latest !== undefined)
                        updateTaskState(entryView.taskKey, { latestAction: latest.action });
                    ...
                    if (entryView.taskKey === visibleTaskKey && activeViewByTask.get(entryView.taskKey) === viewId) {
                        ...
                        if (trail !== undefined)
                            operations.push({ op: 'trail.append', taskKey: entryView.taskKey, entry: trail });
            ...
            case 'createView': {
                        ...
                        if (binding.name === '__dshBrowserTaskAction' && typeof binding.payload === 'string') {
                            try {
                                ...
                                if (action.type === 'switch-task' && typeof action.taskKey === 'string' && activeViewByTask.has(action.taskKey)) {
                                    ...
                                    if (views.get(viewId)?.taskKey === visibleTaskKey)
                                        pushVisibleChromeState();
                                }
                                    ...
                                    if (workspacePanels.tasks && visibleTaskKey !== undefined)
                                        scheduleVisibleTaskThumbnail(visibleTaskKey);
                                    ...
                                    if (task !== undefined)
                                        queueChromePatch({ op: 'task.upsert', task });
                                }
                            }
                ...
                const viewIds = taskViewIds.get(taskKey) ?? new Set();
                viewIds.add(viewId);
            ...
            case 'destroyView': {
                ...
                if (entry !== undefined) {
                    ...
                    if (viewIds !== undefined && viewIds.size === 0)
                        taskViewIds.delete(entry.taskKey);
                    dialogLogs.delete(viewId);
            ...
            case 'getTask': {
                ...
                if (typeof key !== 'string')
                    throw new Error('getTask missing key');
            ...
            case 'updateTask': {
                const key = msg.key;
                if (typeof key !== 'string' || msg.task === undefined)
                    throw new Error('updateTask missing key or task');
