...
function resolveElectronPath() {
    ...
    const addResolvedModule = (resolved) => {
    ...
    try {
        addResolvedModule(require.resolve('electron'));
    }
    ...
    for (const anchor of anchors) {
        try {
            addResolvedModule(require.resolve('electron', { paths: [anchor] }));
        }
        catch { /* keep probing */ }
    }
    const roots = new Set([
...
export function selectSupportedElectronPath(candidates) {
    ...
    throw new Error('dsh-browser-plus requires Electron ' + SUPPORTED_ELECTRON_VERSION +
...
function versionOfElectronExecutable(executable) {
...
class RemoteView {
    ...
    async clearDialog() {
        ...
        return this.client.call('drainDialog', { viewId: this.id });
    }
...
export class RemoteElectronViewHost {
    ...
    async listWindows() {
        ...
        if (client === undefined)
            throw new Error('browser host unavailable');
        const r = await client.call('listWindows');
        return r.windows;
    }
